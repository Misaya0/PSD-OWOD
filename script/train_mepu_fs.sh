...
python train_net.py --dist-url auto --num-gpus 4 --config-file configs/REW/rew_t1_sowod.yaml \
--task S-OWODB/rew_t1	OUTPUT_DIR output/rew/sowod_t1  MODEL.WEIGHTS output/rew/model_final.pth  \

#assign REW scores for unknown pseudo labels 
python train_net.py --rew-inference --resume --dist-url auto --num-gpus 4 \
--rew-config-file configs/REW/rew_t1_sowod.yaml --config-file configs/S-OWODB/t1.yaml \
--task S-OWODB/t1 OUTPUT_DIR output/rew/sowod_t1 \
