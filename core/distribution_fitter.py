...
def get_distribution(record_bg, record_kn, picture_name = None):
    ...
    if picture_name != None:
        ...
        # 找到前景分布的左边界
        for idx, x_val in enumerate(x):
            if y_kn[idx] > 0.05:  # 根据概率密度大于 0.05 判断
                foreground_left_boundary = x_val
                break

        # 保存前景左边界到文件
        with open(os.path.join("vis_dist", "foreground_left_boundary.txt"), "w") as f:
            f.write(f"Foreground Left Boundary: {foreground_left_boundary}\n")
        ...
        plt.close()
    return y_bg, y_kn, foreground_left_boundary
