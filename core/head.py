...
class DynamicHead(nn.Module):
    ...
    def forward(self, features, init_bboxes, t, init_features):
        ...
        ###下面5行：使用RPN开启
        # bs = len(features[0])
        # boxes_tensors = [init_bbox.get('proposal_boxes').tensor for init_bbox in init_bboxes]
        # bboxes = pad_sequence(boxes_tensors, batch_first=True, padding_value=0.0)#使用 pad_sequence 自动填充 0 使得长度一致
        # bboxes = torch.stack(boxes_tensors) # 生成的候选框
        # num_boxes = bboxes.shape[1]
...
class RCNNHead(nn.Module):
    ...
    def forward(self, features, bboxes, pro_features, pooler):
        ...
        # self_att.  自注意力机制+残差连接
        pro_features = pro_features.view(N, nr_boxes, self.d_model).permute(1, 0, 2)
        pro_features2 = self.self_attn(pro_features, pro_features, value=pro_features)[0]
        pro_features = pro_features + self.dropout1(pro_features2)
        pro_features = self.norm1(pro_features)

        # inst_interact. 候选框特征与RoI特征的交互+残差连接
        ...
        for cls_layer in self.cls_module:
            cls_feature = cls_layer(cls_feature)#分类层输出  分类 logits
        for reg_layer in self.reg_module:
            reg_feature = reg_layer(reg_feature)#回归层输出  回归 delta
        if self.disentangled == 0:
            class_logits = self.class_logits(cls_feature)
            objectness = torch.sigmoid(class_logits).sum(dim=-1)  # actually not used
        elif self.disentangled == 1:  # separate head  解耦分类
            class_logits = self.class_logits(cls_feature)
            objectness = torch.sigmoid(self.object_logit(cls_feature))
        else:  # feature orthogonality  特征正交
            ...
            #分解物体特征
            obj_feature = cls_feature.norm(dim=-1, keepdim=True)#幅度
            cls_feature = self.norm4(cls_feature)#方向
            class_logits = self.class_logits(cls_feature)
            class_logit_max = class_logits[:, :-1].max(dim=-1, keepdim=True).values#用于稳定计算概率，避免数值过大或过小
            class_logits -= class_logit_max#稳定 softmax 计算，避免指数计算中的数值溢出问题。
            tmp = class_logits[:, :-1].exp().sum(dim=-1, keepdim=True)#每个框的所有“已知”类别的概率之和
            # assign unknown logit s.t. p(unknown) = p_original(known - max_known)
            unknown_logit = (tmp - 1).log() + tmp.log() - (1 + class_logits[:, -1:].exp()).log()#为未知类别分配logits，使它的概率符合要求
            class_logits = torch.cat([class_logits[:, :-1], unknown_logit], dim=-1)
            objectness = torch.sigmoid(self.object_logit(obj_feature))#计算候选框中是否有物体的置信度
            ...
            if len(self.classes) > 1:
                ...
                class_logits_splits = torch.split(class_logits, self.classes, dim=-1)#根据类别分割logits
                weight_cat = []
                for i in range(len(self.classes)):
                    weight_cat.append(class_logits_splits[i][:, i, :].max(dim=-1, keepdim=True)[0])#存储每个类别的最大logits
                routing_weights = F.gumbel_softmax(torch.cat(weight_cat, dim=-1), dim=-1, hard=True) #选择保留校准层中哪一层的输出
    ...
    def apply_deltas(self, deltas, boxes):
        ...
        if not (pred_boxes[:,2:] >= pred_boxes[:,:2]).all():
            print("(pred_boxes[:,2:] >= pred_boxes[:,:2]).not().all()")
